...
MULTI_DB_TEST_SCRIPT="test_multi_db.py"
...
run_multi_db_tests() {
    print_status "INFO" "Multi-database and directory cleanup tests"
    if python3 "$MULTI_DB_TEST_SCRIPT"; then
        print_status "SUCCESS" "Multi-database and directory cleanup tests"
        return 0
    else
        print_status "ERROR" "Multi-database and directory cleanup tests failed"
        return 1
    fi
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --multi-db-only      Run only multi-database and directory cleanup tests"
...
main() {
    ...
    [ $RUN_MULTI_DB -eq 1 ] && { tests_run=$((tests_run + 1)); run_multi_db_tests && tests_passed=$((tests_passed + 1)) || exit_code=1; }
